      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            {Number(c.entrada) > 0 && `${formatBRL(Number(c.entrada))} de entrada + `}
          ...
          <p className="mt-1 text-xs">
            {quitada ? (
              ...
              <span className="text-muted-foreground">
                1ª parcela em{" "}
                <strong className="text-foreground">{formatMes(c.mes_inicial)}</strong> · {c.parcelas}{" "}
                parcelas, {formatBRL(falta)} no total
              </span>
            ) : (
              <span className="text-muted-foreground">
...
function EditarCompraDialog({
  ...
  const [entrada, setEntrada] = useState(Number(c.entrada) > 0 ? String(c.entrada) : "");
  const [parcelas, setParcelas] = useState(String(c.parcelas));
  const [venc, setVenc] = useState(
    c.primeiro_vencimento ?? `${c.mes_inicial}-${String(c.dia_vencimento ?? 1).padStart(2, "0")}`,
  );
  const invalidar = useInvalidarTudo();
  ...
  const previa = useMemo(
    () => (nParc >= 2 && valorTotal > valorEntrada ? (valorTotal - valorEntrada) / nParc : 0),
    [nParc, valorTotal, valorEntrada],
  );
  ...
  const mut = useMutation({
    mutationFn: () => {
      ...
      if (valorEntrada >= valorTotal)
        throw new Error("A entrada tem que ser menor que o valor total.");
      if (!venc) throw new Error("Informe a data do primeiro vencimento.");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            As parcelas são recalculadas do começo, a partir do vencimento que você informar.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            <Label htmlFor="cp-venc" className="text-xs">
            ...
            <Input
              ...
              className="tabular h-11 w-44"
            />
          </div>
          {previa > 0 && (
            <p className="text-xs text-muted-foreground">
              Fica {valorEntrada > 0 ? `${formatBRL(valorEntrada)} de entrada mais ` : ""}
              {nParc}x de <strong className="text-foreground">{formatBRL(previa)}</strong>.
            </p>
          )}
        </div>
        <DialogFooter>
